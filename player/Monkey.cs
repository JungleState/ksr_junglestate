...
public class Monkey : BaseMonkey {
    ...
    public override Move nextMove(GameState state) {
        ...
        if (lastDir.isMoveable() && state.getCell(lastDir).isFree() && 4 > r.Next(5)) {
            return new Move(Action.MOVE, lastDir, state.round, "De Matt het min code klaut");
        }
        ...
        lastDir = direction;
        return new Move(Action.MOVE, direction, state.round, "ERROR 404: Direction not found");
    }
